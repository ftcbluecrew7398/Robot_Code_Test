...
public abstract class OpenCVPipeline_Front implements CameraBridgeViewBase.CvCameraViewListener2 {
    static {
    ...
    /**
     * Initializes the OpenCVPipeline_Front, but implicitly uses the rear camera.
    ...
    /**
     * Initializes the OpenCVPipeline_Front.
     * @param context the application context, usually hardwareMap.appContext
     * @param viewDisplay the ViewDisplay that will display the underlying JavaCameraView to the screen;
     *                    in most cases, using CameraViewDisplay.getInstance() as the argument is just fine.
     ...
     */
    public void enable() {
        if (!initStarted) throw new IllegalStateException("init() needs to be called before an OpenCVPipeline_Front can be enabled!");
